d3 = function() {
  ...
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, point = [ radius, 0 ], notHemisphere = abs(cr) > ε, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
  ...
  function d3_geo_conicConformal(φ0, φ1) {
    ...
    function forward(λ, φ) {
      var ρ = abs(abs(φ) - π / 2) < ε ? 0 : F / Math.pow(t(φ), n);
  ...
  function d3_geom_voronoiCloseCells(extent) {
    var x0 = extent[0][0], x1 = extent[1][0], y0 = extent[0][1], y1 = extent[1][1], x2, y2, x3, y3, cells = d3_geom_voronoiCells, iCell = cells.length, cell, iHalfEdge, halfEdges, nHalfEdges, edge, start, end;
    while (iCell--) {
      cell = cells[iCell];
